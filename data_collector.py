...
class Camera():
    ...
    def __init__(self):
        ...
        self.color_name = ['red', 'orange', 'yellow', 'green', 'blue']
        self.color_value = [self.red, self.orange, self.yellow, self.green, self.blue]
    ...
    def detectBlocksInDepthImage(self):
        ...
        COLOR_COLLECTIONS = {'red': '0', 'green': '1', 'blue': '2', 'yellow': '3', 'purple': '4', 'orange': '5'}
        CURRENT_COLOR = 'orange'

        canvas = np.zeros((720, 1280))

        for center in top_centers:
            cv2.circle(canvas, center, radius=7, color=(255,0,0), thickness=cv2.FILLED)

        

        data_point = np.where(canvas != 0)

        features = []
        labels = []

        # with open(CURRENT_COLOR + '_data.txt') as f:
        for x, y in zip(data_point[0], data_point[1]):
            RGB_value = img_rgb[x][y].reshape(3)
            HSV_valule = cv2.cvtColor(RGB_value.reshape(1, 1, 3), cv2.COLOR_RGB2HSV).reshape(3)
            LAB_valule = cv2.cvtColor(RGB_value.reshape(1, 1, 3), cv2.COLOR_RGB2LAB).reshape(3)

            color_feature = list(RGB_value) + list(HSV_valule) + list(LAB_valule)
            features.append(color_feature)
            labels.append(COLOR_COLLECTIONS[CURRENT_COLOR])

        for center in top_centers:
            cv2.circle(img_rgb, center, radius=10, color=(255,0,0))

        cv2.imwrite('color_'+CURRENT_COLOR+'_data.png', img_rgb)
        with open('color_'+CURRENT_COLOR+'_data.pkl', 'w') as f:
        ...
        for rect, center, color, area in zip(top_rects, top_centers, colors, top_areas):
            box = cv2.boxPoints(rect)
            box = np.int0(box)
            cv2.drawContours(img_rgb, [box], 0, (255, 0, 0), 1)
            cv2.circle(img_rgb, tuple(list(center)), 2, (255, 0, 0), thickness=-1)
            cv2.putText(img_rgb, 'Large ' + color + str(area), (center[0] - 30, center[1] + 40), 1, 1.0, (0, 0, 0),
                        thickness=2)
