...
class Camera():
    ...
    def detectBlocksInDepthImage(self):
        ...
        self.combined_detection_results = []
        for center, size, color, rect, area in zip(top_centers, top_sizes, colors, top_rects,top_areas):
            center_world = self.img2wolrd(center)[0][:3] / 1000
            # print('center world:', center_world[:3])
            yaw = np.deg2rad(rect[2])
            self.combined_detection_results.append([center_world, size, color, yaw, area])

        print(len(self.combined_detection_results))

        with open(str(EXP_INDEX) + '_data.pkl', 'w') as f:
            pickle.dump(self.combined_detection_results, f)

        cv2.imwrite(str(EXP_INDEX) + '_img.png', cv2.cvtColor(img_rgb, cv2.COLOR_BGR2RGB))
